...
#' @export
eg00_estimating_a_mean_10 <- list(
  ...
  mean of the population from which the data were drawn using a random subsample of 10 cases.",
  ...
  sim_bot = function(data) {
    jsonlite::toJSON(list(
      ...
      payload = data.frame(mean = mean(sample(
        data$y, 10, replace = TRUE
      ))),
  ...
  make_summary = function(results_data) {
    data.frame(
      Quantity = c(
        "Average estimated mean",
        "Std. dev. of the estimated means",
        "Number of estimates"
      ),
      Value = c(
        mean(results_data$mean, na.rm = TRUE),
